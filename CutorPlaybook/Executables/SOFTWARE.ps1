...
function Install7Zip {
    ...
    # Detectar arquitectura (x64 = 0, ARM64 = 1)
    $arm = [Environment]::Is64BitOperatingSystem -and [Environment]::GetEnvironmentVariable("PROCESSOR_ARCHITECTURE") -eq "ARM64"
    $7zipArch = ('x64', 'arm64')[$arm]

    Write-Output "Obteniendo enlace de descarga para 7-Zip $7zipArch..."
    $download = $website + ((Invoke-WebRequest $website -UseBasicParsing).Links.href | Where-Object { $_ -like "a/7z*-$7zipArch.exe" })
    
    if (-not $download) {
        Write-Error "No se pudo encontrar el instalador de 7-Zip en el sitio web."
        exit 1
    }

    $installerPath = "$tempDir\7zip.exe"
    Write-Output "Descargando 7-Zip desde: $download"
    & curl.exe -L -s $download -o $installerPath

    if (Test-Path $installerPath) {
        Write-Output "Instalando 7-Zip..."
        Start-Process -FilePath $installerPath -ArgumentList '/S' -Wait -WindowStyle Hidden
        Write-Output "7-Zip se instaló correctamente."
    } else {
        Write-Error "No se pudo descargar el instalador de 7-Zip."
    }
}
